...
- Remote cloning: 원래 기기의 프로세스는 그대로 실행되게 놔둬
  [2026-04-05-DS_1.md](2026-04-05-DS_1.md)

##### Migration 할때의 resource의 종류

Process Migration을 하는데 TCP port를 움겼다고 해서 복사한 그 포트를 바로 쓸수 있는게 아니라 따로 처리를 해줘야 겠지(e.g. Make global link)

<br> <br>

Resource를 구분하는데 쓰이는 cat도 2개가 있어
- Process-to-**resource** binding
  - Binding by identifier: unique id인거 만 가능(얘 아니면 안돼)
  - Binding by value: id가 달라도 value만 같으면 사용할수 있어 (file content만 같으면 ok 입니다.)
  - Binding by type: File type만 같으면 ok 입니다. 
- Resource-to-**machine** binding
  - Unattached resources: Can be easily moved
  - Fastened resources: HW에 묶인건 아니라 이동은 가능하나 cost가 높다 많이
  - Fixed resources: Bound to specific machine or env

요지랄인데 이해하면 당연한거 아닐까?(?)
![local-resource-cat.png](../assets/DistributedSystem/local-resource-cat.png)

#### Code Migration in heterogeneous systems

system이 다르니 통째로 옮길수 밖에 없을거야: 3 Ways to handle

1. 돌고있는 와중에 Memory 복사하고, 바뀔때마다 새로운 메모리를 계속 복사해
2. Virtual Machine 끄고 migrate하고 새로운 virtual machine start하기
3. 그때 그때 필요한 부분만 실시간으로 복사하기 (바로 실행할순 있어도, 기존 virtual machine을 오랜 시간동안 켜놔야 하겠지) 
